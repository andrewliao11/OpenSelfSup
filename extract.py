...
class ExtractProcess(object):
    ...
    def __init__(self,
        ...
        assert len(layer_indices) == 1, "Avoid using MultiPooling"
    ...
    def _forward_func(self, model, **x):
        backbone_feats = model(mode='extract', **x)[0]
        pooling_feats = self.pooling(backbone_feats).squeeze()
        feat_dict = {"feat1": pooling_feats.cpu()}
...
def parse_args():
    ...
    parser.add_argument(
        '--layer-ind',
        type=str,
        default="4", # https://github.com/andrewliao11/OpenSelfSup/blob/master/openselfsup/models/backbones/resnet.py#L419
...
def main():
    ...
    mmcv.mkdir_or_exist(osp.abspath(cfg.work_dir))
    # logger
    log_file = osp.join(cfg.work_dir, 'extract.log')
    ...
    mmcv.mkdir_or_exist("{}/features/".format(cfg.work_dir))
    if rank == 0:
        for key, val in outputs.items():
            output_file = "{}/features/{}/{}_{}.npy".format(
                cfg.work_dir, args.split, args.pretrained.split("/")[-1], key)
            mmcv.mkdir_or_exist(osp.abspath(osp.dirname(output_file)))
